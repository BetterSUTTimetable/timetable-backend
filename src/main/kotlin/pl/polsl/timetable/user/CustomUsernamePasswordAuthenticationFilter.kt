...
class CustomUsernamePasswordAuthenticationFilter(
    ...
    override fun attemptAuthentication(request: HttpServletRequest, response: HttpServletResponse): Authentication {
        return if (request.contentType?.startsWith("application/json") == true) {
            try {
                val userData = ObjectMapper().readValue(request.reader, UserLoginData::class.java)
                val authRequest = UsernamePasswordAuthenticationToken(userData.email, userData.password)
